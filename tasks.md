...
- [x] **SKE-4.1** 提问斜杠权重回流：根据历史点击、AI 使用和有帮助反馈动态调整提问词推荐顺序
...
- [x] **SKE/PET-2** 后台 Skill 来源统计：区分斜杠提问、AI 回答、学伴、搜索等来源，辅助评估学伴动作效果
- [x] **SKE/PET-3** 学伴动作效果回流：学伴触发 AI 后记录 `ai_used`，并支持有帮助 / 没帮助反馈
- [x] **SKE/PET-4** 后台 Skill 来源筛选：管理端支持按斜杠、AI 回答、学伴、搜索来源筛选 Skill
- [x] **SKE/PET-5** 学伴动作反馈排序：根据本地 Skill 点击、AI 使用、有帮助 / 没帮助反馈，优先展示更有效的学伴动作
- [x] **SKE/PET-6** 高效学伴动作进入题目页：题目教练场展示按反馈排序的学伴建议，点击后带当前题目打开 AI 浮窗
- [x] **SKE/PET-7** 后台低效学伴动作观察清单：后台聚合最近 30 天学伴来源事件，显示没帮助偏高或无正反馈的 Skill
- [x] **SKE/PET-8** 后台 entry_point 来源细分：区分学伴面板、题目页建议、AI 浮窗和反馈按钮入口
- [x] **SKE/PET-9** 学伴 Skill 推荐调整建议：后台观察清单显示建议降权、建议改写、调整入口、保留观察及原因
- [x] **SKE/PET-10** 管理员一键调权草案：建议降权的学伴 Skill 支持人工确认后调低 20% 权重
- [x] **SKE/PET-11** 建议改写入口草案：建议改写的学伴 Skill 支持人工编辑内容并复用现有保存接口
- [x] **SKE/PET-12** 后台轻量操作提示：Skill 管理页临时记录最近人工调权 / 改写操作，刷新后清空
...
- [x] **PET-2** 接入有效学习行为：做对题、订正错题、掌握知识点、完成今日任务增加 XP
- [x] **PET-2.1** 学生端 XP 规则透明化：学伴面板新增成长规则折叠区，说明哪些有效学习行为加 XP
- [x] **PET-3** 接入 AI 状态：AI 生成中 thinking，完成后 review，失败后恢复 idle
- [x] **PET-4** 接入教学 Skill：宠物动作按钮提供“给我一个突破口 / 判断错因 / 来一道更简单的”
- [x] **PET-5** 后端同步：`pet_states` + `pet_events`，支持跨设备和防刷奖励（本地最小实现，migration 草案未执行 / 未部署）
- [x] **PET-6** 成长图鉴与家长端：展示本周成长、修复错因、掌握知识点，不做焦虑排名
- [x] **PET/SKE-1** 学伴动作接入 Skill 事件：记录“突破口 / 错因 / 简单题”点击来源，进入后续效果评估
...
- [x] **V3.5** 学习建设记录模板化（日记式文风 + 截图 + 自媒体复用）
...
- [x] **V4.76** SKE/PET 后台 Skill 行内最近操作提示：Skill 列表行内显示最近降权 / 改写留痕
- [x] **V4.77** SKE/PET 后台 Skill 操作历史入口：Skill 行内可打开最近操作历史，持久化优先、本地降级
- [x] **V4.78** SKE/PET 后台操作历史详情展开：历史记录可展开查看操作前后摘要与原因
- [x] **V4.79** SKE/PET 后台操作历史中文摘要：详情展开改为中文摘要，显示权重、字数和改写预览变化
- [x] **V4.80** SKE/PET 后台操作历史类型筛选：Skill 历史面板支持全部 / 降权 / 改写筛选
- [x] **V4.81** SKE/PET 后台操作日志上线验证前检查清单：明确服务器授权、数据库备份、migration、后端部署、验证和回滚边界
...
- [x] **V4.98** 后台操作日志公网真实写读验收：短期管理员 token 调用公网 API 写入 1 条测试日志，按 `requestId` 读回命中并清理，最终 `admin_operation_logs=0`
...
> V5.15 现实口径校准：
> - W1 的历史勾选代表“规划与本地代码能力已形成”，不能直接等同于“生产 ClawBot 进程已运行、定时推送已线上闭环”
> - 2026-05-09 公网只读核验显示：`/api/wechat/status`、`POST /api/wechat/unbind`、`/api/daily`、`/api/parent/children` 已挂到线上后端，未登录/无效 token 均正确返回 401
> - 2026-05-09 公网只读核验显示：`GET/PUT /api/study-plan` 线上仍为 404，PagePlan 的服务端保存链路不能按已上线处理
> - SSH 免密登录服务器失败，PM2 中是否已有 `shumai-bot`、线上 DB 是否具备微信上下文字段与 `study_plans` 表，还需要后续安全登录后只读核验

### WX. 微信 ClawBot 主路径收口（V5.15 重新归并）

> 目标：把“学生微信扫码 ClawBot → 私聊绑定 → AI 答疑 → 今日计划 → 家长通知 → 定时个性化触达”作为树脉 AI 学习系统的主通道之一重新纳入当前路线，不再散落在 W1、V3、V2、家长端和定时任务里。

- [x] **WX-0** 旧规划归并：确认 V3/W1/V2/L/T1 已经覆盖微信绑定、每日任务、家长摘要和定时推送思路，但需要重新按生产可用性分层
- [~] **WX-1** 线上只读基线：公网已确认微信/每日任务/家长接口存在；PM2 bot 进程、DB schema、服务器源码目录仍待 SSH 安全登录核验
- [ ] **WX-2** ClawBot 运行态核验：确认 `shumai-bot` 是否在线、是否自动重连、是否能收发测试私聊消息
- [ ] **WX-3** 绑定闭环验收：测试账号 H5 登录 → 微信发送“绑定 手机号” → `wechat_uid/context_token/last_seen` 写入 → H5 状态同步
- [ ] **WX-4** 学生指令验收：`今日任务`、`错题`、`学情`、自由问答均能读取学生数据并返回克制、可执行的教练式回复
- [ ] **WX-5** 主动推送 dry-run：先只写日志不群发，验证早 7:00、晚 21:00、周报、遗忘复习、考前节点的候选人和文案
- [ ] **WX-6** 家长通知闭环：孩子学习摘要 → 家长绑定关系 → 家长微信提醒，加入频率控制、免打扰和退订机制
- [ ] **WX-7** 多模态分阶段：文字先闭环；语音、图片/PDF、OCR 与 TTS 作为增强项分批验收，避免一次性把高风险能力全推到生产
- [ ] **WX-8** 运营安全边界：只做学习服务触达，不做广告群发；所有主动消息限频、可关闭、可追踪失败原因
...
- [x] **V2.1** 学生画像采集页（年级/目标分数/每日可用时间/薄弱科目自评/最近状态）
- [x] **V2.2** AI 生成个性化学习路线图（PagePlan V2 前端先基于画像+错题+知识图谱本地生成，DeepSeek 服务端增强留后续）
  · 周目标（第1周补xx，第2周强化yy...）
  · 每日练习计划（具体题目+知识点+时长）
  · 阶段里程碑（每2周一个检查点）
- [x] **V2.3** 学习路线图可视化（阶段卡 + 指标摘要 + 优先修复知识点）
- [x] **V2.4** 里程碑检查（阶段性小测、路线校准、输出稳定性检查）
- [x] **V2.5** 信心建设系统（稳定指数 + 掌握进度 + 可完成每日闭环）
- [x] **V2.6** 家长版学习计划（简化家长摘要，说明本周怎么陪、别只催题量）
- [x] **V2.7** 计划动态调整（根据最近状态、错题数量和每日时长调整任务强度）
...
- [x] **V4.99** 部署脚本专项校准：统一 `/opt/shumai/dist` 与 release 目录事实，修正 `deploy/` 和部署文档中的旧路径假设
- [x] **V5.0** 部署文档最终口径复核：修正 release PM2 启动说明、后端脚本 release 目录要求和前端 `rsync --delete` 发布语义
- [x] **V5.1** GitHub 推送与服务器只读核验：V4.99/V5.0 已推送，公网 health、PM2、Nginx root、release 目录只读核验通过
- [x] **V5.2** release dry-run 预案复核：服务器工具、目录、磁盘、GitHub 访问只读核验通过，校准 PM2 日志路径口径
- [x] **V5.3** release dry-run 执行：新建 release，clone main，前端构建、后端依赖和 3002 旁路 health 均通过，未切换线上
- [x] **V5.4** release dry-run 后复核：确认 PM2 只剩正式进程、3002 无监听、线上 asset 未变、V5.3 release 干净
- [x] **V5.5** 正式发布前操作卡：沉淀发布前核验、备份、同步、验证、回滚和停止条件，不执行发布
- [x] **V5.6** 前端静态正式发布：已将 `/opt/shumai/dist` 正式同步到 V5.3 release 产物并完成健康与 PM2 复核
- [x] **V5.7** 正式发布后页面验收：首页与后台入口壳层正常，health 正常，PM2 仍仅正式进程
- [x] **V5.8** Bug-1/Bug-2 加固 + V2 PagePlan 前端 MVP：刷题筛选持久化、返回高亮重试、个性化学习计划入口已接入
- [x] **V5.9** 本地页面验收与交互微调：PagePlan 头部加入返回数学驾驶舱，构建通过
- [x] **V5.10** PagePlan 移动端间距优化：缩紧卡片留白、两列指标卡，本地页面验收通过
- [x] **V5.11** PagePlan 移动端按钮组收口：今日任务按钮改成固定列数，按钮手感更整齐
- [x] **V5.12** PagePlan 顶部摘要密度微调：补充执行概览条，第一屏更像学习控制台
- [x] **Bug-1** PagePractice 筛选条件导航后丢失：筛选状态保留在 App 层并写入 `localStorage`
- [x] **Bug-2** 真题刷题跳转详情后返回无法定位来源题目：`highlightQId` 返回后滚动高亮，并在渲染延迟时重试
- [x] **V2** PagePlan 个性化学习计划模块：前端 MVP 已完成，DeepSeek 服务端生成留后续增强
- [x] **V5.13** SKE 提问斜杠权重回流：推荐菜单开始结合近 90 天历史反馈重排，前端保留回流分数
- [x] **V5.14** 通用链路审计脚本沉淀：新增可复用的知识点-基础题-题组-压轴-真题审计工具，后续小学/高中也可沿用
- [x] **V3.5** 学习建设记录模板化：统一阶段记录模板，方便截图引用和自媒体复用
- [x] **V5.15** ClawBot 微信主路径统一规划与线上只读核验：归并旧规划、当前代码和公网事实，确认微信/每日任务/家长接口已上线，PM2 bot 与 DB 深核验待安全 SSH
- [x] **V5.16** 树博士前端命名统一：首页 Pet 标题改为“我的伙伴”，网站前端“问学长 / 树脉学长 / AI 学长”统一为“树博士”
- [x] **V5.17** V5.16 部署前线上只读核验：确认公网仍是旧 asset，V5.16 尚未部署；SSH 免密失败，前端发布需先取得安全服务器登录方式
- [x] **V5.18** 树博士前端静态发布：新建 release 构建并同步到 `/opt/shumai/dist`，公网 asset 切到 `index-D0g_6lWZ.js`，旧称无命中
- [x] **V5.19** 树博士上线后页面级验收：公网桌面/移动端真实页面验证 Pet、AI 浮窗、微信 AI 页和练题页新文案，截图入档
- [x] **V5.20** ClawBot 生产只读深核验：只读确认公网接口、PM2、Nginx、生产代码和 DB 基线，明确 bot 进程尚未在线、study-plan API/部分 DB 闭环结构未上线
