...
/* ════════════════════════════════════════════════════════════
   EXAM QUESTIONS — 2015-2025 青岛中考精选
════════════════════════════════════════════════════════════ */
/* ════════════════════════════════════════════════════════════
   数脉 中考真题库 — 多城市精选（2015-2025）
   city: 青岛/北京/上海/广州/深圳/成都/武汉/全国/浙江/江苏
   subTopics: 分拆涉及的所有知识点（用于双向图谱跳转）
════════════════════════════════════════════════════════════ */
const EXAM_QS = [
  /* ── 青岛卷 2015 ─────────────────────────────── */
...
/* ── 题目双向图谱工具函数 ─────────────────────────────────── */
// 做错一道题 → 找到需要补差的基础知识点（向下追因）
function getDownLinks(topicId, masteredSet) {
  const { unmastered, roots } = traceRootCause(topicId, masteredSet||new Set());
  return { unmastered, roots, basics: roots.map(rid=>({
    topic: TOPIC_MAP[rid],
    qs: (BASICS_BY_TOPIC[rid]||[]).slice(0,3),
  })).filter(x=>x.topic) };
}

// 掌握一个知识点 → 找到可以进阶的题组和压轴（向上提升）
function getUpLinks(topicId) {
  const g = GRAPH[topicId] || { groupIds:[], finalIds:[] };
  return {
    groups: g.groupIds.map(id=>TOPIC_GROUPS.find(tg=>tg.id===id)).filter(Boolean),
    finals: g.finalIds.map(id=>FINAL_GROUPS.find(fg=>fg.id===id)).filter(Boolean),
    nextTopics: (g.next||[]).map(id=>TOPIC_MAP[id]).filter(Boolean),
  };
}

// 分解一道复合题 → 列出涉及的所有知识点
function decomposeQuestion(q) {
  const topics = [q.topic, ...(q.subTopics||[])].filter(Boolean);
  return topics.map(id=>TOPIC_MAP[id]).filter(Boolean);
}
...
/* ════════════════════════════════════════════════════════════
   BASIC QUESTIONS — 全部24个知识点基础习题
════════════════════════════════════════════════════════════ */
/* ════════════════════════════════════════════════════════════
   数脉 基础习题库 — 精选题（每题含双向图谱标签）
   upLink  → 做对后推荐的题组/压轴题ID
   downLink→ 做错后补差的前置知识点ID
════════════════════════════════════════════════════════════ */
const BASICS_BY_TOPIC = {
  rational:[
    ...
    {id:"b_rat_04",diff:1,content:"计算：(-4)×(-3)",answer:"12",sol:"①负×负=正；②答案：12",error:"同号得正，异号得负",upLink:["tg15"],downLink:[]},
    {id:"b_rat_05",diff:1,content:"计算：(-18)÷6",answer:"-3",sol:"①负÷正=负；②答案：-3",error:"除法符号判断",upLink:["tg15"],downLink:[]},
    {id:"b_rat_06",diff:2,content:"计算：(-2)×(-3)+(-1)²",answer:"7",sol:"①按运算顺序逐步计算；②6+1=7",error:"(-1)²=1不是-1",upLink:["tg15"],downLink:[]},
    {id:"b_rat_07",diff:2,content:"比较：-|-3|与-(-2)",answer:"-3<2",sol:"-|-3|=-3；-(-2)=2",error:"双重否定处理",upLink:["tg15"],downLink:[]},
    {id:"b_rat_08",diff:2,content:"计算：(-1/2)³÷(-1/8)",answer:"1",sol:"(-1/8)÷(-1/8)=1",error:"分数乘方",upLink:["tg15"],downLink:[]},
    {id:"b_rat_09",diff:2,content:"科学计数法表示0.000036",answer:"3.6×10⁻⁵",sol:"①小数点右移5位；②答案：3.6×10⁻⁵",error:"指数正负方向",upLink:["tg15"],downLink:[]},
    {id:"b_rat_10",diff:2,content:"数轴上A对应-3，B对应2，AB距离=？",answer:"5",sol:"①|2-(-3)|=5；②答案：5",error:"距离=|大-小|",upLink:["tg15"],downLink:[]},
    ...
    {id:"b_rat_13",diff:3,content:"若a和b互为相反数，c和d互为倒数，求a+b+cd",answer:"1",sol:"①a+b=0，cd=1；②答案：1",error:"相反数之和=0，倒数之积=1",upLink:["tg15"],downLink:[]},
    {id:"b_rat_14",diff:3,content:"|x+1|+|y-2|=0，求x+y",answer:"1",sol:"①各=0；x=-1，y=2；②答案：1",error:"非负数之和=0则各为0",upLink:["tg15"],downLink:[]},
    ...
    {id:"b_rat_17",diff:3,content:"ab<0且a+b>0，则a、b的关系",answer:"异号且正数绝对值更大",sol:"ab<0→异号；和>0→正数大",error:"从不等式推断符号",upLink:["tg15"],downLink:[]},
    {id:"b_rat_18",diff:2,content:"已知x=-3，y=2，求|x|-y²+xy",answer:"-7",sol:"①按运算顺序逐步计算；②3-4-6=-7",error:"代入时注意xy是乘法",upLink:["tg15"],downLink:[]},
    {id:"b_rat_19",diff:3,content:"若|a-3|+(b+2)²=0，求a-b",answer:"5",sol:"a=3，b=-2；a-b=5",error:"非负数之和=0则各=0",upLink:["tg15"],downLink:[]},
    {id:"b_rat_20",diff:2,content:"用科学计数法表示1.2×10⁻³，是多少？",answer:"0.0012",sol:"①小数点左移3位；②答案：0.0012",error:"负指数=小数",upLink:["tg15"],downLink:[]},
    {id:"b_rat_21",diff:1,content:"计算：(-2)×3-(-4)÷2",answer:"-4",sol:"①按运算顺序逐步计算；②-6+2=-4",error:"减负数变加",upLink:["tg15"],downLink:[]},
  ...
  reals:[
    ...
    {id:"b_re_02",diff:1,content:"√16=？　-√25=？　√(-9)=？",answer:"4；-5；无意义",sol:"平方根非负；负数无实数平方根",error:"√16≠±4",upLink:["tg06"],downLink:[]},
    ...
    {id:"b_re_06",diff:2,content:"5<√n<6，n的整数取值",answer:"26到35",sol:"①25<n<36；②答案：26到35",error:"边界开区间不含25和36",upLink:["tg06"],downLink:[]},
    ...
    {id:"b_re_09",diff:3,content:"有理化：1/√3",answer:"√3/3",sol:"①乘以√3/√3；②答案：√3/3",error:"有理化分母",upLink:["tg06"],downLink:[]},
    ...
    {id:"b_re_12",diff:1,content:"∛(-8)=？　∛27=？",answer:"-2；3",sol:"(-2)³=-8；3³=27",error:"立方根可以为负数",upLink:["tg06"],downLink:[]},
    {id:"b_re_13",diff:3,content:"若√a+√b=0，求a+b",answer:"0",sol:"√a≥0且√b≥0，各=0",error:"平方根非负，和=0则各=0",upLink:["tg06"],downLink:[]},
    ...
    {id:"b_re_18",diff:2,content:"实数a的平方根是±3，a=？",answer:"9",sol:"①±√9=±3；②答案：9",error:"平方根±√a",upLink:["tg06"],downLink:[]},
  ...
  poly:[
    ...
    {id:"b_pol_03",diff:2,content:"展开：(x+3)(x-2)",answer:"x²+x-6",sol:"①FOIL法；②答案：x²+x-6",error:"中间项漏写",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_pol_07",diff:2,content:"已知a+b=3，ab=2，a²+b²=？",answer:"5",sol:"(a+b)²-2ab=9-4=5",error:"用完全平方转化",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_pol_13",diff:3,content:"若a²+b²=5，a+b=3，(a-b)²=？",answer:"1",sol:"(a-b)²=9-4×2=1",error:"ab用和与平方和推导",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_pol_16",diff:3,content:"若x=√2+1，x²-2x+1=？",answer:"2",sol:"=(x-1)²=(√2)²=2",error:"代入前先化简",upLink:["tg14"],downLink:[]},
    {id:"b_pol_17",diff:2,content:"展开：(a-b)(a²+ab+b²)",answer:"a³-b³",sol:"①立方差公式；②答案：a³-b³",error:"认识立方差",upLink:["tg14"],downLink:[]},
  ...
  factoring:[
    ...
    {id:"b_fac_17",diff:3,content:"分解：x³+3x²+3x+1",answer:"(x+1)³",sol:"①完全立方展开式；②答案：(x+1)³",error:"认识立方展开式",upLink:["tg14"],downLink:[]},
  ...
  fraction:[
    {id:"b_fra_01",diff:1,content:"化简：(x²-4)/(x-2)（x≠2）",answer:"x+2",sol:"①约分（注x≠2）；②答案：x+2",error:"约分后忘限制条件",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_fra_05",diff:2,content:"计算：(x²-4)/x÷(x-2)",answer:"(x+2)/x",sol:"÷(x-2)=×1/(x-2)；约分",error:"除法变乘法再约分",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_fra_09",diff:2,content:"求分式(x+3)/(x-2)有意义的x",answer:"x≠2",sol:"①分母x-2≠0；②答案：x≠2",error:"分式有意义：分母≠0",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_fra_13",diff:3,content:"化简：(1/x-1/y)÷(1/x²-1/y²)",answer:"xy/(x+y)",sol:"①通分后相除；②答案：xy/(x+y)",error:"复合分式化简",upLink:["tg14"],downLink:[]},
  ...
  linear_eq:[
    {id:"b_leq_01",diff:1,content:"解方程：2x+3=7",answer:"x=2",sol:"①2x=4，x=2；②答案：x=2",error:"移项变号",upLink:["tg15"],downLink:[]},
    {id:"b_leq_02",diff:1,content:"解方程：3x-1=2x+4",answer:"x=5",sol:"①x=5；②答案：x=5",error:"移项正负号",upLink:["tg15"],downLink:[]},
    {id:"b_leq_03",diff:2,content:"解方程：2(x-1)=3x+1",answer:"x=-3",sol:"2x-2=3x+1；x=-3",error:"去括号后移项",upLink:["tg15"],downLink:[]},
    {id:"b_leq_04",diff:2,content:"解：x/2-(x-1)/3=1",answer:"x=4",sol:"×6：3x-2(x-1)=6；x=4",error:"去分母后展开括号",upLink:["tg15"],downLink:[]},
    {id:"b_leq_05",diff:2,content:"甲乙共60颗糖，甲是乙的2倍",answer:"甲40，乙20",sol:"①3x=60，x=20；②答案：甲40，乙20",error:"设谁为x",upLink:["tg15"],downLink:[]},
    {id:"b_leq_06",diff:2,content:"某数×3-5=该数+7",answer:"6",sol:"①3x-5=x+7，x=6；②答案：6",error:"题意转化",upLink:["tg15"],downLink:[]},
    ...
    {id:"b_leq_18",diff:2,content:"解：5(x-2)=2(x+1)",answer:"x=4",sol:"5x-10=2x+2；x=4",error:"去括号移项",upLink:["tg15"],downLink:[]},
  ...
  quad_eq:[
    ...
    {id:"b_qeq_12",diff:2,content:"解：x²=5",answer:"x=±√5",sol:"①直接开方；②答案：x=±√5",error:"有两个根±√5",upLink:["tg02","tg03"],downLink:[]},
  ...
  equations:[
    ...
    {id:"b_eqs_12",diff:3,content:"父年龄是子4倍，10年后是2倍",answer:"父40，子10",sol:"f=4s；f+10=2(s+10)",error:"未来年龄=现在+t",upLink:["tg14"],downLink:[]},
  ...
  inequality:[
    {id:"b_ineq_01",diff:1,content:"解：3x-1>5",answer:"x>2",sol:"①3x>6；②答案：x>2",error:"移项变号",upLink:["tg01"],downLink:[]},
    {id:"b_ineq_02",diff:1,content:"解：-2x<6",answer:"x>-3",sol:"①除以-2变号！；②答案：x>-3",error:"除以负数变号",upLink:["tg01"],downLink:[]},
    ...
    {id:"b_ineq_07",diff:2,content:"解：-1≤2x-3≤5",answer:"1≤x≤4",sol:"①加3再除2；②答案：1≤x≤4",error:"三元不等式同步操作",upLink:["tg01"],downLink:[]},
    ...
    {id:"b_ineq_20",diff:2,content:"某工厂计划生产不超过500件产品，已生产了120件，还能继续生产多少件？列不等式",answer:"x≤380",sol:"120+x≤500；x≤380",error:"'不超过'用≤，'至少'用≥",upLink:["tg01"],downLink:[]},
  ],
  coords:[
    {id:"b_coo_01",diff:1,content:"A(3,-2)在第几象限？",answer:"第四象限",sol:"①x>0，y<0；②答案：第四象限",error:"象限判断",upLink:["tg13"],downLink:[]},
    {id:"b_coo_02",diff:1,content:"A(2,3)关于x轴对称点",answer:"B(2,-3)",sol:"①y取反；②答案：B(2,-3)",error:"x轴对称纵坐标取反",upLink:["tg13"],downLink:[],
  ...
  <text x="30" y="160" fill="#dce8f8" font-size="12" font-family="sans-serif">x轴对称：y取反</text>
    ...
    {id:"b_coo_04",diff:2,content:"A(1,2)和B(4,6)的中点",answer:"(5/2,4)",sol:"①各坐标均值；②答案：(5/2,4)",error:"中点公式",upLink:["tg13"],downLink:[],
     svg:`<svg width="195" height="175" viewBox="0 0 195 175" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="28" y="46" fill="#dce8f8" font-size="12" font-family="sans-serif">ȳ=(2+6)/2=4</text>
    ...
    {id:"b_coo_05",diff:2,content:"A(0,0)到B(3,4)的距离",answer:"5",sol:"①√(9+16)=5；②答案：5",error:"距离公式",upLink:["tg13"],downLink:[],
     svg:`<svg width="200" height="185" viewBox="0 0 200 185" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="40" y="88" fill="#fbbf24" font-size="14" font-family="sans-serif">AB=5</text>
    ...
    {id:"b_coo_06",diff:2,content:"A(0,0)，B(4,0)，C(0,3)的三角形面积",answer:"6",sol:"底×高/2=4×3/2=6",error:"直角三角形面积",upLink:["tg13"],downLink:[]},
    ...
    {id:"b_coo_08",diff:2,content:"M是AB中点，A(1,-2)，M(3,1)，B=？",answer:"B(5,4)",sol:"①中点公式逆用；②答案：B(5,4)",error:"中点逆用",upLink:["tg13"],downLink:[],
     svg:`<svg width="200" height="165" viewBox="0 0 200 165" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="30" y="150" fill="#fbbf24" font-size="13" font-family="sans-serif">B=(2×3-1, 2×1-(-2))=(5,4)</text>
    ...
    {id:"b_coo_09",diff:2,content:"A(-1,2)，B(3,-2)，中点M，M关于原点对称N=？",answer:"N(-1,0)",sol:"M=(1,0)；N=(-1,0)",error:"先求中点再求对称",upLink:["tg13"],downLink:[]},
    ...
    {id:"b_coo_12",diff:2,content:"A(2,3)，B(-1,7)，|AB|=？",answer:"5",sol:"①√(9+16)=5；②答案：5",error:"两点距离",upLink:["tg13"],downLink:[]},
    {id:"b_coo_13",diff:3,content:"判断A(1,2)，B(3,6)，C(5,10)是否共线",answer:"共线",sol:"①斜率均=2且有公共点；②答案：共线",error:"三点共线判断",upLink:["tg13"],downLink:[]},
    ...
    {id:"b_coo_15",diff:2,content:"△OAB，O(0,0)，A(4,0)，B(2,3)，面积=？",answer:"6",sol:"S=½×4×3=6（高=B的纵坐标）",error:"坐标系三角形面积",upLink:["tg13"],downLink:[]},
  ...
  linear_fn:[
    {id:"b_lfu_01",diff:1,content:"y=3x-2，x=2时y=？",answer:"4",sol:"①代入计算；②解得4",error:"代入出错",upLink:["tg01","tg14"],downLink:[]},
  ...
  <line x1="80" y1="85" x2="110" y2="85" stroke="#fbbf24" stroke-width="1" stroke-dasharray="3,2"/>
    ...
    {id:"b_lfu_06",diff:2,content:"y=kx+b过一三不过二象限",answer:"k>0，b=0",sol:"①正比例函数；②答案：k>0，b=0",error:"不过二四象限b=0",upLink:["tg01","tg14"],downLink:[]},
    {id:"b_lfu_07",diff:2,content:"y=3x+b过(1,5)，b=？",answer:"b=2",sol:"①5=3+b；②答案：b=2",error:"代入已知点",upLink:["tg01","tg14"],downLink:[]},
    {id:"b_lfu_08",diff:2,content:"y=2x-4，x从-1到2，Δy=？",answer:"Δy=6",sol:"①Δy=k×Δx=2×3；②答案：Δy=6",error:"Δy=k×Δx",upLink:["tg01","tg14"],downLink:[]},
    {id:"b_lfu_09",diff:3,content:"y=kx+b过一二四象限",answer:"k>0，b<0",sol:"过四不过三：b<0；递增：k>0",error:"四象限分析",upLink:["tg01","tg14"],downLink:[]},
    {id:"b_lfu_10",diff:3,content:"f(x)=ax+b，f(1)=3，f(3)=7，f(5)=？",answer:"11",sol:"k=2，b=1；f(5)=11",error:"等差性质",upLink:["tg01","tg14"],downLink:[]},
  ...
  <text x="75" y="150" fill="#fbbf24" font-size="11" font-family="sans-serif">同号必过！</text>
    ...
    {id:"b_lfu_15",diff:2,content:"y=3x+b向下平移2单位",answer:"y=3x+b-2",sol:"①截距减2；②答案：y=3x+b-2",error:"平移只改变截距",upLink:["tg01","tg14"],downLink:[]},
    ...
    {id:"b_lfu_18",diff:2,content:"y=ax+b过原点且斜率=-2",answer:"y=-2x",sol:"①过原点b=0；②答案：y=-2x",error:"过原点即正比例函数",upLink:["tg01","tg14"],downLink:[]},
    ...
    {id:"b_lfu_20",diff:2,content:"y=kx（k≠0）的图像特征",answer:"过原点的直线",sol:"①b=0过原点；②答案：过原点的直线",error:"正比例函数图像",upLink:["tg01","tg14"],downLink:[]},
  ],
  inverse_fn:[
    {id:"b_inv_01",diff:1,content:"y=6/x，x=2时y=？x=-3时y=？",answer:"3；-2",sol:"①代入；②解得3；-2",error:"代入计算",upLink:["tg14"],downLink:[]},
    {id:"b_inv_02",diff:1,content:"y=-4/x在哪象限？分支内y随x如何变",answer:"二、四象限；各分支内递增",sol:"k<0在二四；k<0单调递增",error:"k<0分支内递增",upLink:["tg14"],downLink:[]},
    {id:"b_inv_03",diff:2,content:"y=k/x过(2,-3)，k=？",answer:"k=-6",sol:"①k=xy=2×(-3)；②答案：k=-6",error:"k=x₀y₀",upLink:["tg14"],downLink:[]},
    {id:"b_inv_04",diff:2,content:"y=4/x，P到坐标轴围成矩形面积=？",answer:"4",sol:"①面积=|k|=4；②答案：4",error:"面积恒等定理",upLink:["tg14"],downLink:[]},
    {id:"b_inv_05",diff:2,content:"y=6/x，P到坐标轴围成三角形面积=？",answer:"3",sol:"①|k|/2=3；②答案：3",error:"三角形=|k|/2",upLink:["tg14"],downLink:[]},
    {id:"b_inv_06",diff:2,content:"y与x成反比，x=3时y=4，x=2时y=？",answer:"6",sol:"k=12，y=12/2=6",error:"k=x₁y₁",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_inv_11",diff:2,content:"y=-3/x，P(1,-3)和Q(3,-1)中点",answer:"(2,-2)",sol:"①中点公式；②答案：(2,-2)",error:"中点计算",upLink:["tg14"],downLink:[]},
    ...
    {id:"b_inv_13",diff:2,content:"y=k/x图像在一三象限，点(k,1)在图像上吗？",answer:"在",sol:"①k×1=k✓；②答案：在",error:"代入验证",upLink:["tg14"],downLink:[]},
    {id:"b_inv_14",diff:3,content:"行驶时间t与速度v成反比（距=200km），v=80时t=？",answer:"2.5小时",sol:"①t=200/v；②答案：2.5小时",error:"反比例实际应用",upLink:["tg14"],downLink:[]},
    {id:"b_inv_15",diff:2,content:"y=-2/x+1，x>0时y的范围",answer:"y<1",sol:"x>0时-2/x<0；y<1",error:"正负影响范围",upLink:["tg14"],downLink:[]},
    {id:"b_inv_16",diff:3,content:"P(a,b)在y=k/x上，Q(2a,b/2)在图像上吗？",answer:"在",sol:"(2a)×(b/2)=ab=k✓",error:"xy=k恒成立",upLink:["tg14"],downLink:[]},
  ...
  <text x="40" y="165" fill="#dce8f8" font-size="12" font-family="sans-serif">-1&lt;x&lt;3 时 y&lt;0</text>
    ...
    {id:"b_qfu_13",diff:2,content:"y=x²-2x+m，顶点纵坐标m-1，m=3时最小值=？",answer:"2",sol:"①m-1=2；②答案：2",error:"顶点纵坐标即最小值",upLink:["tg02","tg03"],downLink:[]},
    ...
    {id:"b_qfu_17",diff:2,content:"y=ax²（a>0），x=1时y=2，x=-2时y=？",answer:"8",sol:"①a=2；y=2×4=8；②答案：8",error:"求a后代入",upLink:["tg02","tg03"],downLink:[]},
  ...
  <text x="52" y="108" fill="#dce8f8" font-size="12" font-family="sans-serif">外角=∠A+∠B</text>
    ...
    {id:"b_tri_07",diff:2,content:"△ABC面积=12，BC=8，D是BC中点，△ABD面积=？",answer:"6",sol:"同高，底之比=1:1，面积相等",error:"等底三角形面积比",upLink:["tg04"],downLink:[]},
  ...
  <text x="158" y="100" fill="#dce8f8" font-size="11" font-family="sans-serif">中点</text>
    ...
    {id:"b_tri_10",diff:2,content:"等腰△，AB=AC=5，BC=6，面积=？",answer:"12",sol:"①高=4，S=½×6×4；②答案：12",error:"三线合一，高平分底",upLink:["tg04"],downLink:[]},
    ...
    {id:"b_tri_14",diff:2,content:"△面积为S，a边上的高=？",answer:"ha=2S/a",sol:"①S=½aha；②答案：ha=2S/a",error:"由面积公式求高",upLink:["tg04"],downLink:[]},
  ...
  <text x="68" y="128" fill="#f5a623" font-size="11" font-family="sans-serif">夹角！</text>
    ...
    {id:"b_cong_02",diff:1,content:"SSA能判全等吗？",answer:"不能",sol:"①有反例；②答案：不能",error:"SSA无法判全等",upLink:["tg04"],downLink:[]},
    {id:"b_cong_03",diff:2,content:"∠C=∠F=90°，AC=DF，BC=EF，证全等",answer:"SAS或HL",sol:"①两种方法均可；②答案：SAS或HL",error:"直角三角形HL定理",upLink:["tg04"],downLink:[]},
  ...
  <text x="75" y="168" fill="#dce8f8" font-size="13" font-family="sans-serif">BD=DC</text>
    ...
    {id:"b_cong_10",diff:2,content:"HL定理：Rt△（∠C=90°）AB=DE，AC=DF，证全等",answer:"HL",sol:"①斜边+直角边；②答案：HL",error:"HL只用于直角三角形",upLink:["tg04"],downLink:[]},
    ...
    {id:"b_cong_14",diff:2,content:"全等三角形对应高相等（证明）",answer:"由面积相等和底相等推出",sol:"S=½×底×高；底相等→高相等",error:"面积法证明",upLink:["tg04"],downLink:[]},
  ...
  <text x="13" y="18" fill="#1ed9a0" font-size="13" font-family="sans-serif">A</text>
    ...
    {id:"b_pyth_04",diff:2,content:"矩形8×6，对角线=？",answer:"10",sol:"①√(64+36)=10；②答案：10",error:"对角线用勾股",upLink:["tg06","tg08"],downLink:[],
     svg:`<svg width="210" height="150" viewBox="0 0 210 150" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="14" y="145" fill="#1ed9a0" font-size="13" font-family="sans-serif">D</text>
    ...
    {id:"b_pyth_06",diff:2,content:"坐标系A(1,2)，B(4,6)，|AB|=？",answer:"5",sol:"①√(9+16)=5；②答案：5",error:"两点距离公式",upLink:["tg06","tg08"],downLink:[]},
  ...
  <text x="92" y="172" text-anchor="middle" fill="#dce8f8" font-size="13" font-family="sans-serif">a</text>
    ...
    {id:"b_pyth_09",diff:3,content:"菱形对角线12和16，周长和面积",answer:"周长=40，面积=96",sol:"边=10；面积=d₁d₂/2",error:"菱形对角线垂直平分",upLink:["tg06","tg08"],downLink:[],
     svg:`<svg width="210" height="155" viewBox="0 0 210 155" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="148" y="118" fill="#f5a623" font-size="13" font-family="sans-serif">边=10</text>
    ...
    {id:"b_pyth_16",diff:2,content:"坐标系，三角形ABC坐标法求面积（A(0,0)，B(4,0)，C(2,3)）",answer:"S=6",sol:"①底AB=4，高=3；②答案：S=6",error:"坐标系三角形面积",upLink:["tg06","tg08"],downLink:[]},
    ...
    {id:"b_pyth_18",diff:2,content:"△ABC中，已知三边为a,b,c，若a²+b²<c²，则∠C=？（钝角、锐角还是直角）",answer:"∠C为钝角",sol:"①勾股逆定理的推广；②答案：∠C为钝角",error:"a²+b²与c²大小判断角的类型",upLink:["tg06","tg08"],downLink:[]},
  ...
  special_tri:[
    ...
    {id:"b_spe_02",diff:2,content:"30-60-90，最短边=3，另两边",answer:"3√3和6",sol:"①比1:√3:2；②答案：3√3和6",error:"特殊角三角形边长比",upLink:["tg04","tg06"],downLink:[]},
    {id:"b_spe_03",diff:2,content:"等腰△AB=AC=5，BC=6，底边上的高",answer:"4",sol:"BD=3，AD=√(25-9)=4",error:"三线合一作高",upLink:["tg04","tg06"],downLink:[]},
    {id:"b_spe_04",diff:2,content:"45-45-90，斜边=8，直角边",answer:"4√2",sol:"①2a²=64，a=4√2；②答案：4√2",error:"等腰直角三角形",upLink:["tg04","tg06"],downLink:[]},
    {id:"b_spe_05",diff:2,content:"等腰△周长=20，腰=8，底=？",answer:"4",sol:"①2×8+底=20；②答案：4",error:"周长公式",upLink:["tg04","tg06"],downLink:[]},
    ...
    {id:"b_spe_08",diff:3,content:"等腰△面积=24，底=8，腰=？",answer:"腰=2√13",sol:"高=6；腰²=4²+6²=52",error:"面积求高再勾股",upLink:["tg04","tg06"],downLink:[]},
  ...
  <text x="140" y="125" fill="#f5a623" font-size="13" font-family="sans-serif">边=10</text>
    ...
    {id:"b_quad_06",diff:2,content:"正方形边长=4，E是BC中点，AE=？",answer:"2√5",sol:"AB=4，BE=2；AE=√20",error:"正方形内勾股",upLink:["tg07"],downLink:[]},
    {id:"b_quad_07",diff:3,content:"梯形中位线=？（上底=4，下底=10）",answer:"7",sol:"①(4+10)/2=7；②答案：7",error:"梯形中位线公式",upLink:["tg07"],downLink:[]},
    {id:"b_quad_08",diff:2,content:"平行四边形面积=24，一边=6，这边上的高=？",answer:"4",sol:"①S=底×高；②答案：4",error:"平行四边形面积",upLink:["tg07"],downLink:[]},
    {id:"b_quad_09",diff:3,content:"坐标系ABCD，A(0,0)，B(4,0)，C(4,3)，D(0,3)，是什么四边形",answer:"矩形",sol:"①四边两两相等且相邻垂直；②答案：矩形",error:"坐标判断四边形",upLink:["tg07"],downLink:[]},
    ...
    {id:"b_quad_13",diff:2,content:"菱形周长=20，一对角线=6，另对角线和面积",answer:"另对角线=8，面积=24",sol:"边=5；半对角=√(25-9)=4",error:"菱形边和对角线关系",upLink:["tg07"],downLink:[]},
  ...
  similar:[
    {id:"b_sim_01",diff:1,content:"△ABC∽△DEF，相似比2:3，AB=6，DE=？",answer:"9",sol:"①AB/DE=2/3；②答案：9",error:"相似比方向",upLink:["tg05","tg07"],downLink:[],
  ...
  <text x="80" y="168" fill="#3a9eff" font-size="14" font-family="sans-serif">BC=?</text>
    ...
    {id:"b_sim_04",diff:2,content:"Rt△，CD⊥斜边AB，AD=4，DB=9，CD=？",answer:"6",sol:"①CD²=4×9=36；②答案：6",error:"射影定理",upLink:["tg05","tg07"],downLink:[],
  ...
  <text x="88" y="88" fill="#fbbf24" font-size="13" font-family="sans-serif">CD=?</text>
    ...
    {id:"b_sim_07",diff:2,content:"△ABC中，DE∥BC，AD/DB=1/2，DE=3，BC=？",answer:"9",sol:"AD/AB=1/3；DE/BC=1/3",error:"注意AD/AB不是AD/DB",upLink:["tg05","tg07"],downLink:[]},
    ...
    {id:"b_sim_10",diff:3,content:"△ABC，P在BC上，BP=2，PC=3，S△ABP=4，S△APC=？",answer:"6",sol:"①同高，底之比=面积之比；②答案：6",error:"同高三角形面积比",upLink:["tg05","tg07"],downLink:[]},
  ...
  trig:[
    ...
    {id:"b_trf_04",diff:2,content:"tanα=√3，α=？（锐角）",answer:"60°",sol:"①tan60°=√3；②答案：60°",error:"特殊角",upLink:["tg10"],downLink:[]},
  ...
  <text x="160" y="106" fill="#fbbf24" font-size="14" font-family="sans-serif">h=?</text>
    ...
    {id:"b_trf_12",diff:2,content:"直角△中，30°邻边=√3，斜边=？",answer:"2",sol:"①cos30°=√3/斜边；②答案：2",error:"cos=邻/斜",upLink:["tg10"],downLink:[]},
  ...
  circle:[
    {id:"b_cir_01",diff:1,content:"∠AOB=80°，圆周角∠ACB=？",answer:"40°",sol:"①圆周角=圆心角/2；②答案：40°",error:"圆周角定理",upLink:["tg08","tg09"],downLink:[],
     svg:`<svg width="200" height="190" viewBox="0 0 200 190" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="88" y="155" fill="#fbbf24" font-size="12" font-family="sans-serif">40°</text>
    ...
    {id:"b_cir_02",diff:1,content:"直径AB所对圆周角=？",answer:"90°",sol:"①直径对圆周角是直角；②答案：90°",error:"直径所对圆周角=90°",upLink:["tg08","tg09"],downLink:[]},
  ...
  <text x="45" y="85" fill="#fbbf24" font-size="13" font-family="sans-serif">PA=?</text>
    ...
    {id:"b_cir_04",diff:2,content:"PA、PB是⊙O两切线，PA=6，PB=？",answer:"6",sol:"①切线长定理；②答案：6",error:"同一点两切线等长",upLink:["tg08","tg09"],downLink:[]},
    {id:"b_cir_05",diff:2,content:"内接四边形ABCD，∠A=80°，∠C=？",answer:"100°",sol:"①对角互补；②答案：100°",error:"圆内接四边形对角互补",upLink:["tg08","tg09"],downLink:[],
  ...
  <text x="68" y="98" fill="#dce8f8" font-size="12" font-family="sans-serif">∠A+∠C=180°</text>
    ...
    {id:"b_cir_06",diff:2,content:"弦AB=8，半径=5，弦心距=？",answer:"3",sol:"①d²+16=25；②答案：3",error:"弦心距公式",upLink:["tg08","tg09"],downLink:[],
     svg:`<svg width="200" height="185" viewBox="0 0 200 185" xmlns="http://www.w3.org/2000/svg" style="background:#0d1825;border-radius:8px;display:block">
  ...
  <text x="55" y="180" text-anchor="middle" fill="#dce8f8" font-size="12" font-family="sans-serif">d²+4²=5² → d=3</text>
    ...
    {id:"b_cir_08",diff:2,content:"同弧上两圆周角∠ACB和∠ADB的关系",answer:"相等",sol:"①同弧圆周角相等；②答案：相等",error:"同弧圆周角定理",upLink:["tg08","tg09"],downLink:[]},
    ...
    {id:"b_cir_11",diff:2,content:"圆周长=10π，面积=？",answer:"25π",sol:"①R=5，S=25π；②答案：25π",error:"周长→R→面积",upLink:["tg08","tg09"],downLink:[]},
    ...
    {id:"b_cir_13",diff:2,content:"AB为直径，AC=3，BC=4，∠ACB=90°，R=？",answer:"5/2",sol:"①AB=5，R=5/2；②答案：5/2",error:"直径所对圆周角=90°",upLink:["tg08","tg09"],downLink:[]},
    {id:"b_cir_14",diff:3,content:"两弦AB∥CD，∠AOB=80°，∠COD=？",answer:"80°",sol:"①平行弦截等弧→相等；②答案：80°",error:"平行弦截等弧",upLink:["tg08","tg09"],downLink:[]},
    ...
    {id:"b_cir_17",diff:2,content:"⊙O，P在圆外，PO=10，R=6，PA切线，PA=？",answer:"8",sol:"PA²=PO²-R²=64",error:"切线长公式",upLink:["tg08","tg09"],downLink:[]},
    {id:"b_cir_18",diff:3,content:"证：弦切角=所对弧的圆周角",answer:"弦切角定理",sol:"①标准证明过程；②答案：弦切角定理",error:"弦切角定理",upLink:["tg08","tg09"],downLink:[]},
  ...
  <text x="40" y="168" fill="#dce8f8" font-size="12" font-family="sans-serif">(x,y) → (-y, x)</text>
    ...
    {id:"b_tra_04",diff:2,content:"△ABC绕B点旋转60°，BA=BA'，∠ABA'=？",answer:"60°",sol:"①旋转角=旋转度数；②答案：60°",error:"旋转保持距离和角度",upLink:["tg13"],downLink:[]},
    {id:"b_tra_05",diff:2,content:"y=2x+1向上平移3单位",answer:"y=2x+4",sol:"①截距+3；②答案：y=2x+4",error:"平移改变截距",upLink:["tg13"],downLink:[]},
    {id:"b_tra_06",diff:2,content:"P(a,b)关于y=x的对称点",answer:"P'(b,a)",sol:"①坐标互换；②答案：P'(b,a)",error:"y=x对称互换坐标",upLink:["tg13"],downLink:[]},
  ...
  <text x="55" y="130" fill="#fbbf24" font-size="11" font-family="sans-serif">M是AB中点</text>
    ...
    {id:"b_tra_09",diff:2,content:"位似变换，O为中心，k=2，A(1,3)→A'=？",answer:"A'(2,6)",sol:"①各坐标×2；②答案：A'(2,6)",error:"位似变换坐标",upLink:["tg13"],downLink:[]},
    ...
    {id:"b_tra_13",diff:2,content:"P(2,3)关于直线x=1的对称点",answer:"Q(0,3)",sol:"x'=2×1-2=0；y不变",error:"关于x=a对称：x'=2a-x",upLink:["tg13"],downLink:[]},
    ...
    {id:"b_tra_16",diff:3,content:"P(3,4)关于直线y=2的对称点",answer:"P'(3,0)",sol:"y'=2×2-4=0；x不变",error:"关于y=a对称：y'=2a-y",upLink:["tg13"],downLink:[]},
    ...
    {id:"b_tra_18",diff:3,content:"将函数y=x²向右平移3单位，再向上平移2单位，得到的函数解析式",answer:"y=(x-3)²+2",sol:"右移3：x替换为x-3；上移2：y加2",error:"函数平移：右移x减，上移y加",upLink:["tg13"],downLink:[]},
  ...
  stats:[
    {id:"b_sta_01",diff:1,content:"数据5,3,7,8,2，均值=？",answer:"5",sol:"①25/5=5；②答案：5",error:"均值=总和/个数",upLink:["tg11"],downLink:[]},
    ...
    {id:"b_sta_03",diff:2,content:"数据1,2,3,4,5，方差=？",answer:"2",sol:"均值=3，偏差平方均值=10/5",error:"方差公式",upLink:["tg11"],downLink:[]},
    ...
    {id:"b_sta_11",diff:3,content:"频率直方图各矩形面积之和=？",answer:"1",sol:"①面积=频率，总频率=1；②答案：1",error:"频率总和=1",upLink:["tg11"],downLink:[]},
    ...
    {id:"b_sta_13",diff:3,content:"1000人抽50人系统抽样，间距=？",answer:"20",sol:"①间距=1000/50=20；②=20",error:"系统抽样间距",upLink:["tg11"],downLink:[]},
  ...
  prob:[
    {id:"b_pro_01",diff:1,content:"抛硬币，正面概率=？",answer:"1/2",sol:"①2个结果，1个正面；②答案：1/2",error:"古典概型基础",upLink:["tg12"],downLink:[]},
    {id:"b_pro_02",diff:2,content:"1到10取一个数，偶数概率=？",answer:"1/2",sol:"①5个偶数；②答案：1/2",error:"列举偶数",upLink:["tg12"],downLink:[]},
    ...
    {id:"b_pro_07",diff:2,content:"5红3白取一球，P(红)=？P(白)=？",answer:"5/8，3/8",sol:"①P=有利/总；②答案：5/8，3/8",error:"基本计算",upLink:["tg12"],downLink:[]},
    {id:"b_pro_08",diff:3,content:"抛两次硬币，恰好一次正面的概率",answer:"1/2",sol:"①HT和TH共2种；②答案：1/2",error:"树状图枚举",upLink:["tg12"],downLink:[]},
    {id:"b_pro_09",diff:2,content:"P(A)=0.4，P(B)=0.3，A和B互斥，P(A+B)=？",answer:"0.7",sol:"①互斥直接相加；②答案：0.7",error:"互斥才能相加",upLink:["tg12"],downLink:[]},
    ...
    {id:"b_pro_14",diff:3,content:"抛硬币1000次，正面约多少次",answer:"约500次",sol:"①大数定律；②答案：约500次",error:"频率趋近概率",upLink:["tg12"],downLink:[]},
...
const questionService = {
  ...
  getBasicByTopic(topicCode) { return BASICS_BY_TOPIC[topicCode] || []; },
  getBasicById(id) {
  ...
  return(
    <div style={{padding:isMobile?12:28,maxWidth:1060,margin:"0 auto"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.alg}14,${C.geo}10)`,
        border:`1px solid ${C.alg}25`,borderRadius:18,padding:"26px 32px",marginBottom:20,
        ...
        <div style={{display:"flex",alignItems:"center",gap:20,flexWrap:"wrap"}}>
          <div style={{flex:1}}>
            <div style={{fontSize:15,color:C.alg,letterSpacing:3,marginBottom:8,textTransform:"uppercase",fontWeight:700}}>
              青岛中考数学 · 智能学习系统
            </div>
            <h1 style={{margin:"0 0 8px",fontSize:30,fontWeight:900,color:C.text,letterSpacing:-0.5}}>
              数脉 ShuMai
            </h1>
            <p style={{margin:"0 0 14px",fontSize:17,color:C.muted,lineHeight:1.8}}>
              10年命题知识图谱 · 四模块精准训练 · 23种解题方法 · 做题只做减法，冲满分
            </p>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {MODULE_CONFIG.map(m=>(
                <div key={m.id} style={{padding:"4px 12px",borderRadius:20,fontSize:15,
                  background:m.color+"18",color:m.color,border:`1px solid ${m.color}30`,fontWeight:600}}>
                  {m.icon} {m.label}
                </div>
              ))}
              <div style={{padding:"4px 12px",borderRadius:20,fontSize:15,
          ...
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{fontSize:40,fontWeight:900,color:C.ok,lineHeight:1}}>
              {mastered.size}<span style={{fontSize:22,color:C.muted}}>/{total}</span>
            </div>
            <div style={{fontSize:16,color:C.muted,marginBottom:10}}>知识点已掌握</div>
            <div style={{width:140}}>
              <Bar v={Math.round(mastered.size/total*100)} color={C.ok} h={8}/>
            </div>
            <div style={{fontSize:15,color:C.ok,marginTop:4}}>{Math.round(mastered.size/total*100)}% 完成</div>
          </div>
        </div>
      </div>
      ...
      {!mastered.size&&(
        <button onClick={()=>onNav("quickdiag")}
          style={{width:"100%",display:"flex",alignItems:"center",gap:16,
            padding:"18px 24px",marginBottom:16,borderRadius:14,cursor:"pointer",
          ...
          <span style={{fontSize:15,color:C.cyan,fontWeight:700,flexShrink:0}}>
            开始 →
          </span>
        </button>
      )}
      ...
      {/* Stats row */}
      <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":isTablet?"repeat(3,1fr)":"repeat(5,1fr)",gap:10,marginBottom:16}}>
        {[
          {label:"10年真题",val:EXAM_QS.length,unit:"道",c:C.alg},
          {label:"基础习题",val:500,unit:"道",c:C.m2},
          {label:"题组训练",val:35,unit:"组",c:C.m3},
          {label:"真题错题本",val:wrongSet.size,unit:"题",c:C.red},
          {label:"解题方法",val:23,unit:"种",c:C.purple},
        ].map(s=>(
          <div key={s.label} style={{background:C.s1,border:`1px solid ${s.c}20`,
            borderRadius:10,padding:"14px 16px",textAlign:"center",cursor:"pointer"}}
            onClick={()=>s.label==="错题本"?onNav("wrong"):null}>
            <div style={{fontSize:30,fontWeight:900,color:s.c,lineHeight:1}}>{s.val}</div>
            <div style={{fontSize:15,color:C.muted,marginTop:4}}>{s.label}</div>
          </div>
        ))}
      </div>

      <div style={{display:"grid",gridTemplateColumns:isMobile||isTablet?"1fr":"3fr 2fr",gap:14,marginBottom:14}}>
        {/* Domain progress */}
        <div style={{background:C.s1,border:`1px solid ${C.border}`,borderRadius:12,padding:22}}>
          <h3 style={{margin:"0 0 16px",fontSize:17,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>
            📊 领域掌握进度
          </h3>
          {domStats.map(({k,d,all,done})=>(
            <div key={k} style={{marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:17,color:d.color,fontWeight:600}}>{d.name}</span>
                <span style={{fontSize:16,color:C.muted}}>{done}/{all.length}个</span>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <Bar v={Math.round(done/all.length*100)} color={d.color} h={8}/>
                <span style={{fontSize:16,color:d.color,fontWeight:700,minWidth:36}}>
        ...
        {/* Quick nav */}
        <div style={{background:C.s1,border:`1px solid ${C.border}`,borderRadius:12,padding:22}}>
          <h3 style={{margin:"0 0 14px",fontSize:17,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>
            🚀 快速入口
          </h3>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[
              ...
              <button key={b.v} onClick={()=>onNav(b.v)}
                ...
                onMouseEnter={e=>{e.currentTarget.style.borderColor=b.c+"66";e.currentTarget.style.background=b.c+"0c";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor=b.c+"18";e.currentTarget.style.background=C.s2;}}>
                <div style={{fontSize:16,fontWeight:700,color:b.c,marginBottom:2}}>{b.label}</div>
                <div style={{fontSize:14,color:C.muted}}>{b.desc}</div>
              </button>
            ))}
          </div>
        </div>
    ...
    <div style={{padding:"14px 18px",maxWidth:1200}}>
      ...
      {selTopic&&(
        <div style={{position:"sticky",bottom:0,left:0,right:0,marginTop:8,
          background:C.s2,borderRadius:14,border:`1px solid ${DOM[selTopic.domain]?.color||C.border}`,
          padding:"14px 18px",boxShadow:"0 -4px 20px #0006"}}>
...
function PageModules({mastered,wrongSet,onNav,addWrong,removeWrong,basicWrongSet=new Set(),addBasicWrong=()=>{},removeBasicWrong=()=>{}}) {
  ...
  // TTS 语音讲解状态
  const [ttsId,setTtsId]=useState(null);       // 当前正在朗读的题目id
  const [ttsLoading,setTtsLoading]=useState(null); // 正在生成讲解的题目id
  const [ttsText,setTtsText]=useState({});      // 已生成的讲解文字 {qId: text}
            ...
            <div key={domKey} style={{marginBottom:24}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {TOPICS.filter(t=>t.domain===domKey).map(t=>{
                  ...
                  const basics=BASICS_BY_TOPIC[t.id]||[];
                  return(
      ...
      {activeModule==="m2"&&(
        <div>
          <div style={{background:C.s1,border:`1px solid ${C.m2}22`,borderRadius:12,padding:20,marginBottom:16}}>
            <h3 style={{margin:"0 0 6px",fontSize:20,color:C.m2,fontWeight:800}}>② 基础习题 — 三步以内，接近条件反射</h3>
          ...
          {/* 全部模式 - 知识点习题 */}
          {m2Mode==="all"&&selTopic&&BASICS_BY_TOPIC[selTopic]&&(
            <div>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,flexWrap:"wrap"}}>
                ...
                <span style={{fontSize:16,color:C.muted}}>
                  {BASICS_BY_TOPIC[selTopic].length}道题 · 
                  错{BASICS_BY_TOPIC[selTopic].filter(q=>safeBasicWrong.has(q.id)).length}道
                </span>
                {/* 回到基础知识 */}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {BASICS_BY_TOPIC[selTopic].map((q,i)=>{
                            ...
                            <div style={{marginTop:10,padding:"10px 14px",
                              ...
                              <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10,flexWrap:"wrap"}}>
                                ...
                                {ttsText[q.id]&&ttsId!==q.id&&(
                                  <span style={{fontSize:13,color:C.muted}}>✓ 已缓存，点击再次朗读</span>
                                )}
                              </div>
                              {/* 讲解文字展示 */}
                              {ttsText[q.id]&&(
      ...
      {activeModule==="m3"&&(
        <div>
          <div style={{background:C.s1,border:`1px solid ${C.m3}22`,borderRadius:12,padding:20,marginBottom:16}}>
            ...
            <p style={{margin:0,fontSize:17,color:C.muted,lineHeight:1.8}}>
              题组训练法的核心：每组题目不仅考察单个知识点，更重视题目之间的知识图谱关系。
              通过典型例题构建方法系统，超越单个题目，看到解题方法的体系。
            </p>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:14}}>
            {TOPIC_GROUPS.map(tg=>{
              const cs=tg.topics.map(tid=>DOM[TOPIC_MAP[tid]?.domain]?.color||C.muted);
              const c=cs[0]||C.m3;
              return(
                <div key={tg.id}
                  style={{background:C.s1,border:`1px solid ${c}25`,borderRadius:12,padding:20,cursor:"pointer",
                    transition:"all .2s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor=c+"77"}
                  onMouseLeave={e=>e.currentTarget.style.borderColor=c+"25"}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
                    <div>
                      <div style={{fontSize:15,color:C.m3,fontWeight:700,marginBottom:4,letterSpacing:1}}>
                        题组 {tg.id.slice(2)}
                      </div>
                      <h4 style={{margin:0,fontSize:18,fontWeight:800,color:C.text}}>{tg.name}</h4>
                    </div>
                    <Dots v={tg.diff} c={C.m3}/>
                  </div>
                  <p style={{margin:"0 0 12px",fontSize:16,color:C.muted,lineHeight:1.6}}>{tg.desc}</p>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}}>
                    {tg.topics.map(tid=>(
                      <Tag key={tid} c={DOM[TOPIC_MAP[tid]?.domain]?.color||C.muted} sm>
                        {TOPIC_MAP[tid]?.name}
                      </Tag>
                    ))}
                  </div>
                  ...
                  <div style={{marginTop:12,borderTop:`1px solid ${C.border}`,paddingTop:10}}>
                    {tg.questions.map((q,i)=>{
                      const isOpen=openQ===`${tg.id}_${i}`;
                      return(
                        <div key={i} style={{marginBottom:8}}>
                          <div style={{fontSize:16,color:C.text,lineHeight:1.7,
                            background:C.s2,padding:"8px 10px",borderRadius:7,
                            borderLeft:`2px solid ${C.m3}`,marginBottom:4}}>
                            {i+1}. {q.content}
                          </div>
                          <button onClick={()=>setOpenQ(isOpen?null:`${tg.id}_${i}`)}
                            style={{fontSize:15,padding:"2px 8px",borderRadius:5,cursor:"pointer",
                              background:"none",color:C.sta,border:`1px solid ${C.sta}33`}}>
                            {isOpen?"收起":"解析"}
                          </button>
                          {isOpen&&(
                            <div style={{marginTop:6,background:C.sta+"0a",borderRadius:7,padding:10,
                              border:`1px solid ${C.sta}22`}}>
                              <div style={{fontSize:16,color:C.gold,fontWeight:700,marginBottom:4}}>答案：{q.answer}</div>
                              <div style={{fontSize:16,color:C.text,lineHeight:1.7}}>{q.sol}</div>
                              <div style={{fontSize:15,color:C.red,marginTop:6}}>⚠️ {q.error}</div>
                            </div>
                          )}
                        </div>
                      );
      ...
      {activeModule==="m4"&&(
        <div>
          <div style={{background:C.s1,border:`1px solid ${C.m4}22`,borderRadius:12,padding:20,marginBottom:16}}>
            ...
            <p style={{margin:0,fontSize:17,color:C.muted,lineHeight:1.8}}>
              经过三大模块训练后，进入压轴题训练。综合运用多个知识点和方法，
              在"做题只做减法"的理念下，每道压轴题的突破都代表着解题灵感的升华。
            </p>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
            {FINAL_GROUPS.map(fg=>{
              const c=C.m4;
              return(
                <div key={fg.id} style={{background:C.s1,border:`1px solid ${c}25`,borderRadius:12,padding:22,
                  transition:"all .2s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor=c+"66"}
                  onMouseLeave={e=>e.currentTarget.style.borderColor=c+"25"}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                    <div>
                      ...
                      <h4 style={{margin:"6px 0 0",fontSize:19,fontWeight:800,color:C.text}}>{fg.name}</h4>
                    </div>
                    <Dots v={fg.diff} c={C.m4}/>
                  </div>
                  <p style={{margin:"0 0 12px",fontSize:16,color:C.muted,lineHeight:1.6}}>{fg.desc}</p>
                  ...
                  {fg.questions.map((q,i)=>{
                    const isOpen=openQ===`${fg.id}_${i}`;
                    return(
                      <div key={i}>
                        <div style={{background:C.s2,padding:"12px 14px",borderRadius:8,
                          borderLeft:`3px solid ${C.m4}`,marginBottom:8}}>
                          <p style={{margin:0,fontSize:17,color:C.text,lineHeight:1.8}}>{q.content}</p>
                        </div>
                        <button onClick={()=>setOpenQ(isOpen?null:`${fg.id}_${i}`)}
                          style={{fontSize:16,padding:"4px 12px",borderRadius:6,cursor:"pointer",
                            background:"none",color:C.m4,border:`1px solid ${C.m4}40`,marginBottom:8}}>
                          {isOpen?"▲ 收起解析":"▼ 压轴解析"}
                        </button>
                        {isOpen&&(
                          <div style={{background:C.m4+"0a",border:`1px solid ${C.m4}25`,
                            borderRadius:8,padding:14,marginBottom:8}}>
                            <div style={{fontSize:17,fontWeight:700,color:C.gold,marginBottom:6}}>答案：{q.answer}</div>
                            <div style={{fontSize:17,color:C.text,lineHeight:1.9,whiteSpace:"pre-wrap"}}>{q.sol}</div>
                            <div style={{marginTop:8,fontSize:16,color:C.red}}>⚠️ {q.error}</div>
                          </div>
                        )}
                      </div>
                    );
  ...
  return(
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,
      ...
      <div style={{display:"flex",height:"calc(100vh - 52px)",position:"relative",overflow:"hidden"}}>
        ...
        {/* SIDEBAR */}
        <aside style={{
          width: bp.isMobile ? 240 : bp.isTablet ? 150 : 176,
          ...
          <div style={{padding:"4px 8px 8px"}}>
            <div style={{fontSize:13,color:C.dim,letterSpacing:"1.5px",padding:"6px 4px 5px",textTransform:"uppercase"}}>
              四大模块
            </div>
            {MODULE_CONFIG.map(m=>(
              <button key={m.id} onClick={()=>navigate("modules")}
                style={{width:"100%",textAlign:"left",padding:"7px 10px",borderRadius:7,marginBottom:2,
                  background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:18,fontWeight:900,color:m.color}}>{m.icon}</span>
                <span style={{fontSize:16,color:C.muted}}>{m.label}</span>
              </button>
            ))}
          </div>
          ...
          <div style={{padding:"0 12px 12px"}}>
            <div style={{fontSize:13,color:C.dim,letterSpacing:"1.5px",padding:"6px 0 8px",textTransform:"uppercase"}}>
              掌握进度
            </div>
            {Object.entries(DOM).map(([k,d])=>{
              ...
              return(
                <div key={k} style={{marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:14,color:d.color,marginBottom:3}}>
                    <span>{d.name}</span><span>{done}/{all.length}</span>
                  </div>
                  <Bar v={Math.round(done/all.length*100)} color={d.color} h={4}/>
                </div>
              );
            })}
          </div>
          ...
          <div style={{padding:"0 8px 8px"}}>
            <div style={{fontSize:13,color:C.dim,letterSpacing:"1.5px",padding:"4px 4px 6px",textTransform:"uppercase"}}>
              高频 TOP5
            </div>
            {[...TOPICS].sort((a,b)=>b.freq-a.freq).slice(0,5).map((t,i)=>(
              <button key={t.id} onClick={()=>navigate("detail",t.id)}
                ...
                <span style={{fontSize:14,color:C.muted,minWidth:14}}>{i+1}</span>
                <span style={{fontSize:16,color:C.text,flex:1}}>{t.name}</span>
                <span style={{fontSize:15,color:DOM[t.domain].color,fontWeight:700}}>{t.freq}%</span>
              </button>
            ))}
          </div>
          ...
          <div style={{padding:"0 8px 12px"}}>
            ...
            {METHODS.slice(0,6).map(m=>(
              <button key={m.id} onClick={()=>navigate("methods")}
                style={{width:"100%",textAlign:"left",padding:"4px 8px",borderRadius:5,marginBottom:1,
                  background:"none",border:"none",cursor:"pointer"}}>
                <span style={{fontSize:15,color:C.purple}}>{m.id.slice(1)}</span>
                <span style={{fontSize:15,color:C.muted,marginLeft:4}}>{m.name}</span>
              </button>
            ))}
            <button onClick={()=>navigate("methods")}
              style={{width:"100%",textAlign:"left",padding:"4px 8px",marginTop:2,
                background:"none",border:"none",cursor:"pointer",fontSize:15,color:C.purple}}>
              查看全部23种 →
            </button>
          </div>
          ...
          {/* 错题本快捷 */}
          <div style={{padding:"0 8px 12px"}}>
            <div style={{fontSize:13,color:C.dim,letterSpacing:"1.5px",padding:"4px 4px 6px",textTransform:"uppercase"}}>错题本</div>
            <button onClick={()=>navigate("wrong")}
              style={{width:"100%",textAlign:"left",padding:"6px 10px",borderRadius:7,
                marginBottom:4,background:wrongSet.size>0?C.red+"14":"none",
                border:`1px solid ${wrongSet.size>0?C.red+"33":C.border}`,
                color:wrongSet.size>0?C.red:C.muted,fontSize:15,cursor:"pointer"}}>
              📋 真题错题 {wrongSet.size>0&&<b>{wrongSet.size}道</b>}
            </button>
            <button onClick={()=>{navigate("modules");}}
              style={{width:"100%",textAlign:"left",padding:"6px 10px",borderRadius:7,
                background:basicWrongSet.size>0?C.red+"14":"none",
                border:`1px solid ${basicWrongSet.size>0?C.red+"33":C.border}`,
                color:basicWrongSet.size>0?C.red:C.muted,fontSize:15,cursor:"pointer"}}>
              ② 基础错题 {basicWrongSet.size>0&&<b>{basicWrongSet.size}道</b>}
            </button>
          </div>
        </aside>
