...
V4.51 增加 `server/services/ai-routing.js`，把教学 Skill 的 AI 调用分成内部档位：
...
前端不暴露模型选择。学生只看到学习服务，后端根据 `scene / mode / skill.type / difficulty / question_type / plan` 选择档位。

当前阶段只返回点数预估，不真实扣点，不新增数据库表。后续真实计费可以在订阅体系或 AI usage 表中落账。

## 11. SKE Skill 质量评测

V4.52 在后台 Skill 列表中聚合 `prompt_skill_events` 最近 30 天数据，形成最小质量观察口径。
...
本阶段只展示质量数据，不自动降权。自动降权需要样本量阈值、人工确认和误伤保护。
